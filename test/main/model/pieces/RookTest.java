...
public class RookTest {
     ...
     */
    @Test
    public void validRookSetup() throws Exception {

        int row = 0;
        int col = 0;
        String color = BLACK;
        String type = "R";
        Piece rook = Board.getPiece(row, col);

        assertEquals(color, rook.getColor());
        assertEquals(row, rook.getSquare().getRow());
        assertEquals(col, rook.getSquare().getCol());
        assertEquals(type, rook.getType());
    ...
    public void getAllPossibleMoves() throws Exception {
        ...
        Piece bishop = new Rook(2, 0, BLACK);
